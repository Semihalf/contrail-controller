...
KSyncBase::KSyncBase(Agent *agent)
...
void KSyncBase::RegisterDBClients(DB *db) {
...
void KSyncBase::Init(bool create_vhost) {
    NetlinkInit();
...
void KSyncBase::InitFlowMem() {
    flowtable_ksync_obj_.get()->InitFlowMem();
}

void KSyncBase::NetlinkInit() {
...
int KSyncBase::Encode(Sandesh &encoder, uint8_t *buf, int buf_len) {
    int len, error;
    len = encoder.WriteBinary(buf, buf_len, &error);
    return len;
}

void KSyncBase::VRouterInterfaceSnapshot() {
...
void KSyncBase::VnswInterfaceListenerInit() {
